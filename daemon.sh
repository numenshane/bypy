...
if  [[ `ps -elf|grep "$execDir/bypy.py" |grep upload|wc -l` -eq 0 ]]; then  
    ...
    find . -maxdepth 1 -type f -exec bash -c 'if [ "$1" != "." ]; then /Data/bypy/bypy.py upload "$1" && rm -fr "$1"; fi' -- {} \;  
    find . -maxdepth 1 -type d -exec bash -c 'if [ "$1" != "." ]; then /Data/bypy/bypy.py upload "$1" `date +%Y-%m-%d` && rm -fr "$1"; fi' -- {} \;  
